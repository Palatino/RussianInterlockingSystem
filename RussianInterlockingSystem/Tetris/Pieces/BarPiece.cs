...
namespace TetrisComponents
{
    ...
    {
        public BarPiece(TetrisBoard board_)
            :base(board_, 0)
        {
            centroidColumn = (int)((board_.columns) / 2.0);
            centroidRow = 0;
            configurations = new List<List<int>> {
                new List<int>() { 0, -2, 0, -1, 0, 0, 0, 1 },
                new List<int>() { -1, -1, 0, -1, 1, -1, 2, -1},
                new List<int>() { 1, -2, 1, -1, 1, 0, 1, 1 },
                new List<int>() { -1, 0, 0, 0, 1, 0, 2, 0 },
            };
